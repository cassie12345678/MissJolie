/* ============================================================
   HOME PAGE - BEST SELLING COLLECTIONS
============================================================ */
let strings = {};
let collections = {};
...
// Bestseller collection keys - not used anymore, now filtering by mode
const bestSellerKeys = ["Collectie 1", "Collectie 4", "Collectie 6"];
...
function renderCollections() {
    ...
    // Filter collections by current mode and show first 3
    const filteredCollections = Object.keys(collections)
        .filter(key => {
            const col = collections[key];
            return col && col.active && col.mode === currentMode;
        })
        .slice(0, 3); // Show only 3 collections

    filteredCollections.forEach(key => {
        const col = collections[key];

        const tiers = getActiveCollectionTiers(col);
        const defaultTier = tiers[0];
        const defaultPrice = col.prices[defaultTier];

        // Get translated title and description
        const title = typeof col.title === 'object' ? (col.title[currentLang] || col.title.nl) : col.title;
        const description = typeof col.description === 'object' ? (col.description[currentLang] || col.description.nl) : col.description;

        container.innerHTML += `
            <div class="product-card" data-collection-key="${key}">
                <div class="product-card-media">
                    <img src="${DEFAULT_COLLECTION_IMAGE}" alt="${title}" loading="eager" data-default-tier="${defaultTier}">
                </div>
...
function syncCollectionCardImages() {
    document.querySelectorAll(".product-card[data-collection-key]").forEach(card => {
        ...
        applyCollectionImage(img, col, activeTier, title);
    });
}
...
function enableTierSwitching() {
    document.querySelectorAll(".tier").forEach(btn => {
        btn.onclick = () => {
            ...
            const price = collections[collection].prices[tier];
            card.querySelector(".product-price").textContent = "€" + price;
            
            const col = collections[collection];
            const img = card.querySelector(".product-card-media img");
            const title = card.querySelector("h3")?.textContent || "";
            applyCollectionImage(img, col, tier, title);
        };
    });
}
...
function enableBuyButtons() {
    document.querySelectorAll(".buy-btn").forEach(btn => {
        btn.onclick = () => {
            ...
            const col = collections[collectionKey];
            const title = typeof col.title === 'object' ? (col.title[currentLang] || col.title.nl) : col.title;
            const tierImage = getRenderedCollectionImage(card, col, tier);
...
if (cartPanel) cartPanel.onclick = e => {
    if (e.target === cartPanel) cartPanel.classList.add("hidden");
};
...
function renderCart() {
    ...
    if (cart.length === 0) {
        cartItemsDiv.innerHTML = `<p class="small-text">${strings[currentLang]?.cart_empty || "Nog geen items toegevoegd."}</p>`;
